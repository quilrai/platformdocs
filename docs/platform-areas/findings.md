...
- **Compliance Findings:** Findings and no-risk interactions from compliance integrations such as
  Claude Compliance and OpenAI Compliance, when configured. Policy Engine findings are also
  available here when the Policy Engine license is active.
...
When the Policy Engine license is active, a **Policy Engine** option appears in the provider
switcher. Selecting it opens the Policy Engine Findings view, which shows findings matched by
custom detection rules. Key capabilities:

- **Time-range presets:** Last 15 minutes, 1 hour, 6 hours, 24 hours, 7 days, 30 days, all time,
  or a custom date range.
- **Severity filter:** Narrow results to critical, high, medium, or low severity findings.
- **Trend chart:** Per-day findings count for the selected time range.
- **Findings table:** Rule name, severity badge, tags, matched-event count, and matched timestamp.
- **Detail drawer:** Configured evidence fields resolved from the source event, and raw event data
  as a JSON tree.

Policy Engine findings are generated by detection rules configured in Settings → Compliance → OpenAI.
